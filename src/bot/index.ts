...
// Dedup: prevent Telegram re-delivery on slow tasks
...
function isAdmin(userId: number) { return userId === ADMIN_ID; }
...
function mainMenuKeyboard(isAdminUser: boolean) {
  ...
  if (isAdminUser) rows.push([Markup.button.callback("\u2699\uFE0F \u0644\u0648\u062D\u0629 \u0627\u0644\u0625\u062F\u0627\u0631\u0629", "admin:panel")]);
...
export function createBot(): Telegraf {
  ...
  bot.action("templates:list", async (ctx) => {
    ...
    if (allTemplates.length === 0) {
      await ctx.reply("\u{1F4ED} \u0644\u0627 \u062A\u0648\u062C\u062F \u0642\u0648\u0627\u0644\u0628 \u0645\u062A\u0627\u062D\u0629 \u0628\u0639\u062F.", mainMenuKeyboard(isAdmin(ctx.from!.id)));
      return;
    }
    const rows = allTemplates.map((t) => [Markup.button.callback("\u{1F4C4} " + t.name, "template:select:" + t.id)]);
    rows.push([Markup.button.callback("\u{1F3E0} \u0627\u0644\u0631\u0626\u064A\u0633\u064A\u0629", "menu:main")]);
    await ctx.reply("\u{1F4CB} \u0627\u0644\u0642\u0648\u0627\u0644\u0628 \u0627\u0644\u0645\u062A\u0627\u062D\u0629:", Markup.inlineKeyboard(rows));
  });
  ...
  bot.action(/^template:select:(\d+)$/, async (ctx) => {
    ...
    if (!template) { await ctx.reply("\u26A0\uFE0F \u0627\u0644\u0642\u0627\u0644\u0628 \u063A\u064A\u0631 \u0645\u0648\u062C\u0648\u062F."); return; }
    if (!(await isGoogleAuthorized())) {
      await ctx.reply("\u26A0\uFE0F Google Drive \u063A\u064A\u0631 \u0645\u062A\u0635\u0644.\n\n\u{1F517} \u0627\u0641\u062A\u062D \u0627\u0644\u0631\u0627\u0628\u0637 \u0644\u0631\u0628\u0637 \u0627\u0644\u062D\u0633\u0627\u0628:\n" + getAuthLink());
      return;
    }
    const fields = template.fields as Array<{ key: string; label: string; required: boolean }>;
    if (fields.length === 0) { await ctx.reply("\u26A0\uFE0F \u0647\u0630\u0627 \u0627\u0644\u0642\u0627\u0644\u0628 \u0644\u064A\u0633 \u0644\u0647 \u062D\u0642\u0648\u0644."); return; }
    await upsertSession(userId, { state: "filling", templateId, collectedData: {}, currentFieldIndex: 0 });
    await ctx.reply(
  ...
  bot.action("history:list", async (ctx) => {
    ...
    if (history.length === 0) {
      await ctx.reply("\u{1F4ED} \u0644\u0645 \u062A\u0642\u0645 \u0628\u062A\u0639\u0628\u0626\u0629 \u0623\u064A \u0645\u0633\u062A\u0646\u062F \u0628\u0639\u062F.", mainMenuKeyboard(isAdmin(userId)));
      return;
    }
    let text = "\u{1F4DC} \u0622\u062E\u0631 10 \u0645\u0633\u062A\u0646\u062F\u0627\u062A:\n\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n";
    for (const doc of history) {
      const tmpl = await getTemplate(doc.templateId);
      text += "\u{1F4C4} " + (tmpl?.name || "\u0642\u0627\u0644\u0628 \u0645\u062D\u0630\u0648\u0641") + " \u2014 " + new Date(doc.createdAt).toLocaleDateString("ar-SA") + "\n";
  ...
  bot.action("admin:add_template", async (ctx) => {
    ...
    await upsertSession(userId, { state: "admin", adminState: "waiting_template_name", tempTemplateName: "", tempTemplateFields: [] });
    await ctx.reply("\u2795 \u0625\u0636\u0627\u0641\u0629 \u0642\u0627\u0644\u0628 \u062C\u062F\u064A\u062F\n\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n1\uFE0F\u20E3 \u0623\u0631\u0633\u0644 \u0627\u0633\u0645 \u0627\u0644\u0642\u0627\u0644\u0628:");
  });
  ...
  bot.action("admin:delete_template", async (ctx) => {
    ...
    if (!isAdmin(userId)) return;
    const allTemplates = await getAllTemplates();
    if (allTemplates.length === 0) { await ctx.reply("\u{1F4ED} \u0644\u0627 \u062A\u0648\u062C\u062F \u0642\u0648\u0627\u0644\u0628 \u0644\u062D\u0630\u0641\u0647\u0627."); return; }
    const rows = allTemplates.map((t) => [Markup.button.callback("\u{1F5D1} " + t.name, "admin:confirm_delete:" + t.id)]);
    rows.push([Markup.button.callback("\u{1F3E0} \u0627\u0644\u0631\u0626\u064A\u0633\u064A\u0629", "menu:main")]);
    await ctx.reply("\u{1F5D1} \u0627\u062E\u062A\u0631 \u0627\u0644\u0642\u0627\u0644\u0628 \u0644\u062D\u0630\u0641\u0647:", Markup.inlineKeyboard(rows));
  });
  ...
  bot.action("admin:save_template", async (ctx) => {
    ...
    if (!session?.tempTemplateName) { await ctx.reply("\u26A0\uFE0F \u0644\u0645 \u064A\u062A\u0645 \u0631\u0641\u0639 \u0645\u0644\u0641 \u0627\u0644\u0642\u0627\u0644\u0628 \u0628\u0639\u062F."); return; }
  ...
  bot.on("document", async (ctx) => {
    ...
    if (fileName.toLowerCase().endsWith(".pdf")) {
      ...
      if (fileSizeMB > MAX_PDF_MB) {
        ...
        await ctx.reply(
          "\u26A0\uFE0F \u062D\u062C\u0645 \u0627\u0644\u0645\u0644\u0641 \u0643\u0628\u064A\u0631 \u062C\u062F\u0627\u064B (" + fileSizeMB.toFixed(1) + " MB).\n" +
          "\u0627\u0644\u062D\u062F \u0627\u0644\u0623\u0642\u0635\u0649 \u0627\u0644\u0645\u0633\u0645\u0648\u062D: " + MAX_PDF_MB + " MB.\n" +
          "\u{1F4A1} \u062C\u0631\u0628 \u062A\u0642\u0633\u064A\u0645 \u0627\u0644\u0645\u0644\u0641 \u0625\u0644\u0649 \u0623\u062C\u0632\u0627\u0621 \u0623\u0635\u063A\u0631."
        );
        return;
      }

      const sizeTip = fileSizeMB > 3
        ? " (\u0642\u062F \u064A\u0633\u062A\u063A\u0631\u0642 \u062F\u0642\u064A\u0642\u0629 \u0623\u0648 \u0623\u0643\u062B\u0631)"
        : "";
      await ctx.reply("\u23F3 \u062C\u0627\u0631\u064A \u062A\u062D\u0644\u064A\u0644 \u0645\u0644\u0641 PDF..." + sizeTip);
      ...
      try {
        ...
        const totalBlocks = extracted.pages.reduce((s, p) => s + p.elements.filter(e => e.type === "text_block").length, 0);
        const skippedPages = extracted.pageCount - extracted.pages.length;
        ...
        await ctx.replyWithDocument(
          { source: coordBuffer, filename: fileName.replace(".pdf", "") + "_coordinates.json" },
          {
            caption:
              ...
              "\u{1F4C4} \u0627\u0644\u0635\u0641\u062D\u0627\u062A: " + extracted.pageCount + (skippedPages > 0 ? " (\u062A\u0645\u062A \u0645\u0639\u0627\u0644\u062C\u0629 \u0623\u0648\u0644 50 \u0641\u0642\u0637)" : "") + "\n" +
              "\u{1F4DD} \u0627\u0644\u0646\u0635\u0648\u0635: " + totalBlocks,
          }
        );
        ...
        await ctx.replyWithDocument(
          { source: fillBuffer, filename: fileName.replace(".pdf", "") + "_fill_template.json" },
          {
            caption:
              ...
              "\u0639\u0628\u0626 \u0627\u0644\u0642\u064A\u0645 \u0627\u0644\u0641\u0627\u0631\u063A\u0629 \u062B\u0645 \u0623\u0631\u0633\u0644\u0647 \u0644\u0627\u062D\u0642\u0627\u064B",
          }
        );
      } catch (err: any) {
        ...
        await ctx.reply(
          isTimeout
            ? "\u274C \u0627\u0633\u062A\u063A\u0631\u0642 \u0627\u0644\u062A\u062D\u0644\u064A\u0644 \u0648\u0642\u062A\u0627\u064B \u0637\u0648\u064A\u0644\u0627\u064B.\n\u{1F4A1} \u062C\u0631\u0628 \u0645\u0644\u0641\u0627\u064B \u0623\u0635\u063A\u0631 \u0623\u0648 \u0628\u0639\u062F\u062F \u0635\u0641\u062D\u0627\u062A \u0623\u0642\u0644."
            : "\u274C \u062A\u0639\u0630\u0631 \u062A\u062D\u0644\u064A\u0644 \u0627\u0644\u0645\u0644\u0641. \u062A\u0623\u0643\u062F \u0623\u0646\u0647 \u063A\u064A\u0631 \u0645\u062D\u0645\u064A \u0648\u063A\u064A\u0631 \u062A\u0627\u0644\u0641."
        );
      } finally {
        processingMessages.delete(msgKey);
      }
    ...
    if (session?.state === "admin" && session.adminState === "waiting_template_file" && isAdmin(userId)) {
      if (!fileName.endsWith(".docx")) { await ctx.reply("\u26A0\uFE0F \u064A\u062C\u0628 \u0623\u0646 \u064A\u0643\u0648\u0646 \u0627\u0644\u0645\u0644\u0641 .docx \u0641\u0642\u0637."); return; }
  ...
  bot.on("text", async (ctx) => {
    ...
    if (state === "admin" && isAdmin(userId)) { await handleAdminText(ctx, userId, text, session); return; }
    if (state === "filling" && session?.templateId) { await handleFillingText(ctx, userId, text, session); return; }
  ...
  async function handleAdminText(ctx: Context, userId: number, text: string, session: any) {
    ...
    if (adminState === "waiting_template_name") {
      await upsertSession(userId, { tempTemplateName: text, adminState: "waiting_template_description" });
      await ctx.reply("\u2705 \u0627\u0633\u0645 \u0627\u0644\u0642\u0627\u0644\u0628: " + text + "\n\n2\uFE0F\u20E3 \u0623\u0631\u0633\u0644 \u0648\u0635\u0641\u0627\u064B \u0645\u062E\u062A\u0635\u0631\u0627\u064B (\u0623\u0648 \u0623\u0631\u0633\u0644 - \u0644\u0644\u062A\u062E\u0637\u064A):");
      return;
    }
    ...
    if (adminState === "waiting_field_label") {
      const key = text.replace(/\s+/g, "_").replace(/[^\w\u0600-\u06FF]/g, "").toLowerCase();
      const newFields = [...(session.tempTemplateFields || []), { key, label: text, required: true }];
      await upsertSession(userId, { tempTemplateFields: newFields, adminState: "adding_fields" });
      await ctx.reply(
        "\u2705 " + text + " \u2192 {" + key + "}\n\u0625\u062C\u0645\u0627\u0644\u064A: " + newFields.length,
        Markup.inlineKeyboard([
  ...
  async function handleFillingText(ctx: Context, userId: number, text: string, session: any) {
    ...
    if (!template) { await upsertSession(userId, { state: "idle" }); await ctx.reply("\u26A0\uFE0F \u0627\u0644\u0642\u0627\u0644\u0628 \u063A\u064A\u0631 \u0645\u0648\u062C\u0648\u062F."); return; }
    const fields = template.fields as Array<{ key: string; label: string; required: boolean }>;
    const currentIndex = session.currentFieldIndex || 0;
    const collectedData = (session.collectedData as Record<string, string>) || {};
    collectedData[fields[currentIndex].key] = text;
    const nextIndex = currentIndex + 1;
    if (nextIndex < fields.length) {
      await upsertSession(userId, { collectedData, currentFieldIndex: nextIndex });
      await ctx.reply(
      ...
      try {
        ...
        const uploaded = await uploadFile(outFileName, "application/vnd.openxmlformats-officedocument.wordprocessingml.document", filledBuffer);
        await saveFilledDocument({ templateId: template.id, userId, filledData: collectedData, driveFileId: uploaded.id, driveLink: uploaded.link });
        await ctx.replyWithDocument(
        ...
        logger.error(err, "Error filling document");
        await ctx.reply("\u274C \u062D\u062F\u062B \u062E\u0637\u0623 \u0623\u062B\u0646\u0627\u0621 \u0627\u0644\u062A\u0639\u0628\u0626\u0629.", mainMenuKeyboard(isAdmin(userId)));
      }
    }
  }
  ...
  bot.catch((err: any, ctx) => {
    ...
    if (ctx.chat) ctx.reply("\u26A0\uFE0F \u062D\u062F\u062B \u062E\u0637\u0623. \u0623\u0631\u0633\u0644 /cancel \u0648\u062D\u0627\u0648\u0644 \u0645\u062C\u062F\u062F\u0627\u064B.").catch(() => {});
  });
